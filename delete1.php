<?php
    ...
    echo "<tr>";
        ...
        if(!empty($_GET["col"])){
            foreach($_GET["col"] as $col){
                if($col == 'ID')
                    echo "<p> <font color=white>$col<input type = 'hidden', value = '$key', name = 'primKey' checked> : <input type = 'text' value = '$key' id='datacol' class='data'>";
                else
    ...
    echo "<tr>";
        ...
        $result = mysqli_query($conn, $sql);
			...
			while($row = mysqli_fetch_assoc($result)){
                ...
                echo'<input type="hidden" name="primKey" value='.$row["ID"].'> <input type="submit" name="delete" value="DELETE" id="delete_btn" class= "delete"> <input type="submit" name="update" value="UPDATE" id="update_btn" class= "update"></form></td>';
                echo "</tr>";
            }
        }
        echo "</table>";
